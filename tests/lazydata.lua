...
state.form.values.path = root .. "/missing\ndatabase.db"
vim.api.nvim_feedkeys(string.char(20), "x", false)
assert(vim.wait(3000, function() return state.form and state.form.status_kind == "error" end, 20), "connection test error was not shown")
local error_lines = vim.api.nvim_buf_get_lines(state.form.buf, state.form.status_row - 1, state.form.status_end_row, false)
assert(#error_lines >= 2, "multiline connection error was not rendered on separate lines")
assert(error_lines[1]:find("SQLite database file does not exist", 1, true), "connection error omitted its summary")
assert(error_lines[#error_lines]:find("database.db", 1, true), "connection error omitted its multiline detail")
state.form.values.path = db
vim.api.nvim_feedkeys(string.char(20), "x", false)
...
state.picker.set_filter("")
local printable = {}
for code = 32, 126 do printable[#printable + 1] = string.char(code) end
printable = table.concat(printable)
vim.api.nvim_feedkeys(printable, "x", false)
assert(state.picker and state.picker.filter == printable, "database picker did not accept every printable ASCII key: " .. vim.inspect(state.picker and state.picker.filter))
state.picker.set_filter("")
vim.api.nvim_feedkeys("Košice", "xt", false)
assert(state.picker and state.picker.filter == "Košice", "database picker did not accept Unicode text: " .. vim.inspect(state.picker and state.picker.filter))
state.picker.set_filter("")
state.picker.set_filter("kovac")
vim.api.nvim_feedkeys(vim.keycode("<BS>"), "xt", false)
assert(state.picker and state.picker.filter == "kova", "database picker Backspace inserted a keycode instead of deleting text: " .. vim.inspect(state.picker and state.picker.filter))
vim.api.nvim_feedkeys(vim.keycode("<C-h>"), "xt", false)
assert(state.picker and state.picker.filter == "kov", "database picker Ctrl-H did not delete text: " .. vim.inspect(state.picker and state.picker.filter))
vim.api.nvim_feedkeys(vim.keycode("<C-u>"), "xt", false)
assert(state.picker and state.picker.filter == "", "database picker Ctrl-U did not clear text: " .. vim.inspect(state.picker and state.picker.filter))
state.picker.set_filter("bet")
assert(state.picker and state.picker.filter == "bet", "database picker did not capture filter text")
local filter_line = vim.api.nvim_buf_get_lines(state.picker.buf, state.picker.filter_row - 1, state.picker.filter_row, false)[1]
local filter_cursor = vim.api.nvim_win_get_cursor(state.picker.win)
assert(filter_cursor[1] == state.picker.filter_row and filter_cursor[2] == #filter_line, "database picker cursor was not positioned after the filter text: " .. vim.inspect(filter_cursor))
