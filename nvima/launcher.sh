...
APP_DIR="$CACHE_BASE/nvima-portable/$PAYLOAD_ID"

if [ ! -x "$APP_DIR/nvim/bin/nvim" ]; then
  command -v tar >/dev/null 2>&1 || { echo 'nvima requires tar to unpack itself' >&2; exit 127; }
  TMP_DIR="$APP_DIR.tmp.$$"
  rm -rf "$TMP_DIR"
  mkdir -p "$TMP_DIR"
  ...
  rm -rf "$APP_DIR"
  mv "$TMP_DIR/nvima-payload" "$APP_DIR"
